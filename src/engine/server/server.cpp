...
/* INFECTION MODIFICATION START ***************************************/
	...
	SetClassAvailability(PLAYERCLASS_ZAGE, 1);
...
void CServer::SendCapabilities(int ClientID)
{
	...
	Msg.AddInt(SERVERCAPFLAG_PINGEX | SERVERCAPFLAG_SYNCWEAPONINPUT); // flags
	SendMsg(&Msg, MSGFLAG_VITAL, ClientID);
}
	...
	if(Sys)
	{
		...
		else if(Msg == NETMSG_READY)
		{
			if((pPacket->m_Flags&NET_CHUNKFLAG_VITAL) != 0 && m_aClients[ClientID].m_State == CClient::STATE_CONNECTING)
			{
				...
				char aBuf[256];
				str_format(aBuf, sizeof(aBuf), "player is ready. ClientID=%d addr=%s secure=%s version=%d", ClientID, aAddrStr, m_NetServer.HasSecurityToken(ClientID)?"yes":"no", GetClientVersion(ClientID));
			...
			while(t > TickStartTime(m_CurrentGameTick+1))
			{
				...
				for(int c = 0; c < g_Config.m_SvMaxClients; c++)
				{
					if(m_aClients[c].m_State >= CClient::STATE_READY && m_aClients[c].m_Session.m_MuteTick > 0)
						m_aClients[c].m_Session.m_MuteTick--;
...
int CServer::GetClientVersion(int ClientId) const
{
	...
	if(ClientId == -1)
		return VERSION_DDNET_INDEPENDENT_SPECTATORS_TEAM;
