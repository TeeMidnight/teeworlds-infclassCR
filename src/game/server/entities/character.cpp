...
void CCharacter::UpdateTuningParam()
{
	...
	{
		float Factor = g_Config.m_InfHeroDecreaseSpeed * 0.01f;
		pTuningParams->m_GroundControlSpeed = pTuningParams->m_GroundControlSpeed * (1.0f - Factor);
		pTuningParams->m_GroundControlAccel = pTuningParams->m_GroundControlAccel * (1.0f - Factor);
		pTuningParams->m_GroundJumpImpulse = pTuningParams->m_GroundJumpImpulse * (1.0f - Factor);
		pTuningParams->m_AirJumpImpulse = pTuningParams->m_AirJumpImpulse * (1.0f - Factor);
	...
	case WEAPON_HAMMER:
	{
		...
		else
		{
			...
			if (!AutoFire)
			{
				...
				if(GetClass() == PLAYERCLASS_ZAGE && IsDisguising())
					BreakDisguise();
				...
				for (int i = 0; i < Num; ++i)
				{
					...
					if (IsRealZombie())
					{
						...
						else if (GetClass() == PLAYERCLASS_ZAGE)
						{
							pTarget->TakeDamage(vec2(0.f, -1.f) + normalize(Dir + vec2(0.f, -1.1f)) * 10.0f, g_Config.m_InfZageDamage,
												m_pPlayer->GetCID(), m_ActiveWeapon, TAKEDAMAGEMODE_NOINFECTION);
						}
						else if (GetClass() == PLAYERCLASS_NIGHTMARE)
						{
							pTarget->TakeDamage(vec2(0.f, -1.f) + normalize(Dir + vec2(0.f, -1.1f)) * 10.0f, g_Config.m_InfNightmareDamage,
												m_pPlayer->GetCID(), m_ActiveWeapon, TAKEDAMAGEMODE_NOINFECTION);
						}
					...
					{
						if (pTarget->IsRealZombie())
						{
							if(pTarget->GetClass() == PLAYERCLASS_ZAGE)
...
void CCharacter::Tick()
{
...

	{
		...
		else if (IsRealZombie() && (Index0 == ZONE_DAMAGE_DEATH_INFECTED))
		{
			Die(m_pPlayer->GetCID(), WEAPON_WORLD);
		}
	...
	if(m_DisguisingTimer.has_value())
	{
		m_DisguisingTimer.value()--;
		int DisguisingSec = round_to_int(m_DisguisingTimer.value() / (float)Server()->TickSpeed());
		GameServer()->SendBroadcast_Localization(m_pPlayer->GetCID(), BROADCAST_PRIORITY_EFFECTSTATE, BROADCAST_DURATION_REALTIME, _("You are disguising: {sec:EffectDuration}"), "EffectDuration", &DisguisingSec, NULL);
		if(m_DisguisingTimer.value() <= 0)
			BreakDisguise();
	}
...
void CCharacter::TickDefered()
{
	...
	if (Events & COREEVENT_AIR_JUMP && m_pPlayer->IsCameraOn())
		GameServer()->CreateSound(m_Pos, SOUND_PLAYER_JUMP, CmaskOne(m_pPlayer->GetCID()));
...
bool CCharacter::TakeDamage(vec2 Force, int Dmg, int From, int Weapon, int Mode)
{
	...
	if (GetClass() != PLAYERCLASS_HUNTER ||
		(Weapon != WEAPON_SHOTGUN && pKillerPlayer && pKillerPlayer->GetClass() != PLAYERCLASS_MAGICIAN))
	{
		m_Core.m_Vel += Force;
	}
	...
	if (GetClass() == PLAYERCLASS_NINJA && g_Config.m_InfNinjaForesee == 1 && Weapon == WEAPON_NINJA && pKillerPlayer && pKillerPlayer->IsZombie())
	{
		...
		if (random_prob(DodgeChance))
		{
			...
			m_NinjaForesee = max(0, m_NinjaForesee - g_Config.m_InfNinjaForeseeDecrease);
			m_Armor = m_NinjaForesee;
			return false;
		}
		else
...
void CCharacter::Snap(int SnappingClient)
{
	...
	if (m_LoveTick > 0 || m_HallucinationTick > 0 || m_SlowMotionTick > 0 || IsInNightmare())
...
void CCharacter::ClassSpawnAttributes()
{
	...
	case PLAYERCLASS_ZAGE:
		...
		GameServer()->SendBroadcast_ClassIntro(m_pPlayer->GetCID(), PLAYERCLASS_ZAGE);
		break;
...
void CCharacter::BreakDisguise()
{
	m_DisguisingTimer.reset();
}
...
{
	return m_pPlayer->IsZombie() && !IsDisguising();
}

bool CCharacter::IsHuman() const
{
	return m_pPlayer->IsHuman() || IsDisguising();
}

bool CCharacter::IsRealZombie() const
{
	return m_pPlayer->IsZombie();
}

bool CCharacter::IsRealHuman() const
{
...
void CCharacter::Disguise(int Target)
{
	if(m_Disguised)
		return;
	m_Disguised = true;
	m_DisguisingTimer = std::make_optional(g_Config.m_InfDisguiseTimer * Server()->TickSpeed());
}
/* INFECTION MODIFICATION END *****************************************/