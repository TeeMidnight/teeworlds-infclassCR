...
CElasticHole::CElasticHole(CGameWorld *pGameWorld, vec2 CenterPos, int OwnerClientID)
...
{
	new CGrowingExplosion(GameWorld(), m_Pos, vec2(0.0, -1.0), m_Owner, g_Config.m_InfElasticHoleRadius/32*6, GROWINGEXPLOSIONEFFECT_BOOM_ALL);
...
void CElasticHole::Tick()
{
	...
	if(m_Radius > g_Config.m_InfElasticHoleRadius)
	{
		m_Growing = GROW_STOPING;
		m_Radius = g_Config.m_InfElasticHoleRadius;
	}
	...
	for(CCharacter *pChr = (CCharacter*) GameWorld()->FindFirst(CGameWorld::ENTTYPE_CHARACTER); pChr; pChr = (CCharacter *)pChr->TypeNext())
	{
		...
		if(Len < pChr->m_ProximityRadius+m_Radius)
		{
			...
			pChr->SetVel(vec2(Vel.x*-1.5, Vel.y*-1.5));
		}
	}
	bool boom = false;
	if(m_Radius < 0 && !boom)
